function [dist, theta] = pos2dist_asymmetry(lag1,lon1,lag2,lon2,method)
    ...
    if method == 1
        ...
        %theta is the angle between the vector from the eye to the
        %desired grid and due east
        theta = atan(abs(km_la/km_lo)); 
        
        if lon2 < lon1
            if lag2 < lag1 %Quadrant III
                theta = theta + pi;
            else  %Quadrant II
                theta = pi - theta;
            end
        else
            if lag2 < lag1  %Quadrant IV
                theta = 2*pi - theta;
            end
        end
        ...
        lag2 = lag2 * deg2rad;
        lon2 = lon2 * deg2rad;
        dist = R_aver * acos(min(cos(lag1)*cos(lag2)*cos(lon1-lon2) + sin(lag1)*sin(lag2),1));
        
        
        
        %calculate plane angle (maybe not accurate)
        km_per_deg_la = 111.3237;
        km_per_deg_lo = 111.1350;
        km_la = km_per_deg_la * (lag1-lag2);
        % Always calculate the shorter arc.
